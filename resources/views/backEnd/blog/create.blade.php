<div class="modal-content">            
    <div class="modal-header">
        <h5 class="modal-title" > {{ isset($data->id) ? 'Edit Blog' : 'Add Blog' }} </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div> 
    {!! Form::open(['route'=>'blog.create', 'method' => 'post', 'files' => 'true','class'=>'ajax-form']) !!}
                ...
                <div class="col-12 col-sm-6 col-md-4">
                    <div class="form-group">
                        ...
                        <select name="blog_category_id" class="form-control select2" required >
                            ...
                            @foreach($blogCatrgory as $category)
                                <option value="{{ $category->id }}" {{ isset($data->blog_category_id) && $data->blog_category_id == $category->id ? 'selected' : Null }} >{{ $category->name }}</option>
                            @endforeach
                        </select>     
                    </div>
                </div>  
